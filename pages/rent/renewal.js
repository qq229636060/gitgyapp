			...
			submit: function(fb) {
				...
				if (testmoblie) {
					...
					if (fb == 1) {
						...
						bui.ajax({
							url: apiUrl + "/mapi/tenant/renewSave",
							data: {
								room_id: roomid,
								id: htid,
		...
		mounted: function() {
			...
			bui.ajax({
				...
				if (htid != 0) {
					...
					that.formboxs.timee = getNextDate(result.data.tenant.end_time,1);
					that.formboxs.times = getNextDate(result.data.tenant.end_time,1);
